import { createResourceApi } from '@/apis/resource';
...
function WorkloadAddDrawer({ onClose }: { onClose: () => void }) {
  const [workloadData, setWorkloadData] = useState(`apiVersion: apps/v1
kind: Deployment
metadata:
  ...
  const handleAddWorkload = useMutation({
    ...
    mutationFn: async () => {
      ...
      try {
        ...
        queryClient.invalidateQueries({ queryKey: ['getWorkloadListApi'] });
      } catch (error: unknown) {
  ...
  return (
    <Portal>
      ...
      <Drawer.Positioner>
        <Drawer.Content>
          <Drawer.Header>
            <Drawer.Title>Workload Add</Drawer.Title>
          </Drawer.Header>
          <Drawer.Body>
            <Box height='92%'>
              <Editor
